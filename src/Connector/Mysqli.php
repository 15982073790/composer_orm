...
namespace Mrstock\Orm\Connector;
...
class Mysqli
{
    ...
    protected static function connect($host = 'default', $useRead = true)
    {
        ...
        if (isset(self::$linkPool[$linkName]['handle']) && is_object(self::$linkPool[$linkName]['handle'])) { // 如果已经连接
            if (self::$linkPool[$linkName]['iftransacte'] === false) { // 当前连接启用了事务 则不检查持续时间 也不重新建立连接
                return;
            }
        }
        ...
        if (@mysqli_connect_errno()) {
            ...
            throw new \Exception("Db Error: database connect failed " . @mysqli_connect_errno());
        }
    ...
    protected static function query($sql, $host = 'default', $useRead = true)
    {
        ...
        //echo mysqli_error(self::$linkPool[$linkName]['handle']);exit;
        ...
        Log::write(self::$linkPool[$linkName]['sqling'] . '---' . $linkName . '---' . " [ RunTime:" . $runTime . " 毫秒 ]", LogLevel::SQLRECORD);
    ...
    public static function getAffectedRows($host = 'default')
    {
        ...
        $rows = @self::$linkPool[$linkName]['handle']->affected_rows;
        return $rows;
    }
    ...
    public static function beginTransaction($host = 'default')
    {
        ...
        $linkName = self::getLinkName($host, $useRead);
        // 如果开启事务 则新起连接
        if (isset(self::$linkPool[$linkName]) && is_object(self::$linkPool[$linkName]['handle'])) { // 如果已经连接
            @self::$linkPool[$linkName]['handle']->close();
            unset(self::$linkPool[$linkName]);
        }
