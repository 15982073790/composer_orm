...
class Model
{
    ...
    {
        Db::commit($this->host);
        self::$open_begintransaction = false;
        return true;
    }
     ...
     * @return unknown
     */
    public function setInc($field, $step = 1, $options = array())
    {
        return $this->setField($field, array('exp', $field . '+' . $step), $options);
    }
     ...
     * @return unknown
     */
    public function setDec($field, $step = 1, $options = array())
    {
        return $this->setField($field, array('exp', $field . '-' . $step), $options);
    }

    protected function setField($field, $value = '', $options = array())
    {
        if (is_array($field)) {
            $data = $field;
        } else {
